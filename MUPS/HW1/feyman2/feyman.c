...
double feyman_original(int arc, char **argv)
{
  double ctime_start = cpu_time();
  ...
  printf("\n\nRMS absolute error in solution = %e, time: %f\n", err, ctime_start);
  timestamp();

  return err;
}

double feyman_omp(int arc, char **argv)
{
  double a = 3.0;
  double b = 2.0;
  double c = 1.0;
...
#pragma omp parallel default(none) \
                         private(i,j,k,x,y,z,chk,w_exact,w,ut,x1,x2,x3,vs,trial,dx,dy,dz,vh,we,us) \
                         shared(err,n_inside,wt,ni,nj,nk,a,b,c,seed,stepsz,h,N)
{
#pragma omp single
{
  for (i = 1; i <= ni; i++)
...
#pragma omp task
{
  ...
  printf("\n\nOPENMP: RMS absolute error in solution = %e, time: %f\n", err, ctime);
  timestamp();
...
{
  double error = feyman_original(arc, argv);
  double omp_error = feyman_omp(arc, argv);

  printf("\n");
  if (abs(omp_error - error) <= 0.01)
    printf("TEST PASS\n");
  else
    printf("TEST FAIL\n");
  printf("\n\n\n");
